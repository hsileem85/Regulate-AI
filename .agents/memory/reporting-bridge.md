...
- Validation engine handles `cross_column` (numeric comparison between two columns), `business_check` (semantic presence check), `is_integer` (whole-number only), and `no_numbers` (text-only) in `artifacts/api-server/src/routes/requests.ts`
